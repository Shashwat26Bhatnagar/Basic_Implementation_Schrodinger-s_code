...
# Modify the index route to collect all optimization results
@app.route('/')
def index():
    # Download stock data and create plot
    ...
    exact_result = MinimumEigenOptimizer(exact_mes).solve(qp)
    optimization_results.append({
        'method': 'Classical Exact Solver',
        'assets': [tickers[i] for i, val in enumerate(exact_result.x) if val > 0.5],
        'value': round(exact_result.fval, 4)
    })

    # Quantum SamplingVQE
    cobyla = COBYLA(maxiter=500)
    ry_ansatz = TwoLocal(len(tickers), "ry", "cz", reps=3, entanglement="full")
    svqe_result = MinimumEigenOptimizer(SamplingVQE(Sampler(), ry_ansatz, cobyla)).solve(qp)
    optimization_results.append({
        'method': 'Quantum Sampling VQE',
        'assets': [tickers[i] for i, val in enumerate(svqe_result.x) if val > 0.5],
        'value': round(svqe_result.fval, 4)
    })
    ...
    optimization_results.append({
        'method': 'Quantum QAOA',
        'assets': [tickers[i] for i, val in enumerate(qaoa_result.x) if val > 0.5],
        'value': round(qaoa_result.fval, 4)
    })
